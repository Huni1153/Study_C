...
int hintAlg(NumberBB* nbb)
{
    ...
    for (int col = 0; col < 999; col++) // 출력해줄 값들을 돌릴 viewNum배열에서 하나씩 받아와 자리에 temp배열에 자릿수 별로 나눠 저장한다.
    {
        ...
        {
            for (int hei = 0; hei < 3;hei++) // temp배열을 heil변수로 컨트롤 하기위한 for문
            {
        ...
        /* 입력한 값과 랜덤값 결과의 striked와 ball 을 입력한 값과 출력값 결과의 strike와 ball이 같지 않다면 해당 col위치에 viewNum값을 -1로 초기화
        즉.. */
        if (s != nbb->strike || b != nbb->ball) 
        {
            nbb->viewNum[col] = -1; 
        }
        s = 0;
